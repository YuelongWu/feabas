...
### Combat Alignment Drift

FEABAS uses a connected spring-mesh to spread deformation across the Z-axis. This prevents the 'runaway' distortion often seen in other aligners. However, this system works best when tissue distortions are random. If there is a consistent change in the samples, e.g., a new knife causing a sudden change in compression, the final stack may still show a noticeable drift. 

To mitigate persistent drift, users can perform an iterative reference update. Once a mesh relaxation round is complete, the resulting displacement can be set as the new 'resting state' for a subsequent round. This process allows the system to gradually absorb systematic deformations, as the restorative forces from the initial physical strain diminish with each iteration. Practically, this is achieved by moving the .h5 transformation files from the `tform` folder to replace the files in the `mesh` folder before restarting the optimization. For large datasets, this 'annealing' process is most efficient when performed during the thumbnail alignment step; the corrected global geometry can then be carried over to provide a better starting position for the fine alignment.

A secondary source of drift, primarily occurring during the thumbnail alignment phase, stems from matching precision. When the XY resolution of the thumbnail is significantly lower than the physical section thickness, minor inaccuracies in feature matching can accumulate into a "random walk" error, manifesting as global drift. Fortunately, biological features often exhibit local isotropy at low resolutions: large-scale structures change more slowly across the Z-axis than fine-scale textures. This allows for successful matching between sections that are far apart in Z—axis that is often impossible at full resolution. To counteract random-walk drift, FEABAS supports long-range matching at the thumbnail stage (e.g., matching sections separated in Z by a distance equivalent to the XY pixel size). By anchoring a section to multiple distant neighbors, the cumulative error can be potentially reduced. Since full-resolution matches usually fail over such distances, FEABAS supports separate matching lists. Simply provide a `match_name.txt` file in your `thumbnail_align` folder to include your long-range anchors, and a separate one in the align folder for your fine-scale, adjacent-section matches. Each line in these files define a specific pair to be matched using the format: `source__to__target`.
